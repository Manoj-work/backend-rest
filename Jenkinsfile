        ...
        stage('Build JAR File with Gradle') {
           ...
           stage('Build and Save Podman Image') {
            steps {
                script {
                    try {
                        sh " /home/sudo -u podman -i podman build --platform=linux/amd64 -t ${env.IMAGE_NAME}:${env.IMAGE_TAG} ."
                        sh " /home/sudo -u podman -i podman build  save -o ${env.TAR_FILE} ${env.IMAGE_NAME}:${env.IMAGE_TAG}"
